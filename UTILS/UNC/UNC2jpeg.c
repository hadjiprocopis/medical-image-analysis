...
\na JPEG file for each slice, named as 'outputBasename'_001.jpg etc.";
...
int	main(int argc, char **argv){
	...
	int		quality = 100; /* jpeg output quality */
	...
	for(s=0;s<numSlices;s++){
		...
		if( numSlices > 1 ){
			if( s < 9 ) sprintf(outputFilename, "%s_00%d.jpg", outputBasename, s+1);
			else if( s < 99 ) sprintf(outputFilename, "%s_0%d.jpg", outputBasename, s+1);
			else sprintf(outputFilename, "%s_%d.jpg", outputBasename, s+1);
		} else sprintf(outputFilename, "%s.jpg", outputBasename);

		printf("%d ", slice+1); fflush(stdout);
		if( write_unc_slice_to_grayscale_jpeg_file(data[slice], W, H, quality, outputFilename) == FALSE ){
			fprintf(stderr, "%s : call to write_jpeg_file has failed for input file '%s', slice %d (dimensions %d x %d), output filename was '%s'.\n", argv[0], inputFilename, slice, W, H, outputFilename);
			free(inputFilename); free(outputBasename);
			freeDATATYPE3D(data, actualNumSlices, W);
			exit(1);
		}

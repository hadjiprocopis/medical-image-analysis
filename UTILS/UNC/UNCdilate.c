...
\n\t -o outputFilaname\
\n\
\n\t-n newColor		(Specifies the new pixel value dilated\
\n			 pixels should have.)\
\n\
...
\n			 Dilation is the process by which non-brain\
\n			 pixels become brain pixels!!!)]\
\n\
\n\t[-r iterations	(Specifies the number of passes for the\
\n			 dilation procedure. The default is 1.\
\n			 Make sure that the dilated pixels color\
...
\n			 (slice number, new pixels added to this slice,\
\n			 and total pixels added so far))]\
\n\
\n\t[-p			(Dilate periphery pixels only flag)]\
\n\
\n\t[-R filename\
\n			(write dilated pixels into file 'filename')]\
\n\
\n\t[-9\
...
int	main(int argc, char **argv){
	...
	int		x = 0, y = 0, w = -1, h = -1, W = -1, H = -1, numSlices = 0,
			depth, format, s, slice, totalPixelsDilated = 0,
			pixelsDilated = 0, i, j, r, numIterations = 1, actualNumSlices = 0,
			optI, slices[1000], allSlices = 0, verboseFlag = FALSE,
			dilatePeripheryOnlyFlag = FALSE;
	spec		thresholdSpec;
	char		*reverseFilename = NULL;
	DATATYPE	***dataIn;

	/* default values in erosion.h */
	/* what has pixel intensities between low and high IS BRAIN and will be dilated */
	thresholdSpec.low = DEFAULT_THRESHOLD_LOW_VALUE;
	thresholdSpec.high = DEFAULT_THRESHOLD_HIGH_VALUE;
	thresholdSpec.newValue = -1;
	...
	while( (optI=getopt(argc, argv, "i:o:s:w:h:x:y:t:n:r:vp9R:")) != EOF)
		switch( optI ){
			...
			case 'p': dilatePeripheryOnlyFlag = TRUE; break;
	...
	if( thresholdSpec.newValue == -1 ){
		fprintf(stderr, "Usage : %s %s\n%s\n", argv[0], Usage, Author);
		fprintf(stderr, "The value of dilated pixels should be defined using the '-n' option.\n");
		exit(1);
	}
	...
	for(s=0;s<actualNumSlices;s++) for(i=0;i<W;i++) for(j=0;j<H;j++){
		dataOut[s][i][j] = 0;
		dataIn[s][i][j] = data[s][i][j];
	}

	if( dilatePeripheryOnlyFlag ){
		for(r=0;r<numIterations;r++){
			if( verboseFlag ){ printf("erosion pass %d: ", r+1); fflush(stdout); }
			for(s=0;s<numSlices;s++){
				slice = (allSlices==0) ? slices[s] : s;
				pixelsDilated = dilate_periphery2D(dataIn[slice], x, y, w, h, dataOut[slice], thresholdSpec);
				totalPixelsDilated += pixelsDilated;
				if( verboseFlag ){ printf("(%d,%d,%d) ", slice+1, pixelsDilated, totalPixelsDilated); fflush(stdout);}
		...
		for(r=0;r<numIterations;r++){
			...
			for(s=0;s<numSlices;s++){
				slice = (allSlices==0) ? slices[s] : s;
				pixelsDilated = dilate2D(dataIn[slice], x, y, w, h, dataOut[slice], thresholdSpec);
				totalPixelsDilated += pixelsDilated;
				if( verboseFlag ){ printf("(%d,%d,%d) ", slice+1, pixelsDilated, totalPixelsDilated); fflush(stdout);}
				for(i=0;i<W;i++) for(j=0;j<H;j++) dataIn[slice][i][j] = dataOut[slice][i][j];
			}
		...
		if( verboseFlag ) printf("reversing ...\n");
		for(s=0;s<numSlices;s++){
			slice = (allSlices==0) ? slices[s] : s;
			for(i=0;i<W;i++) for(j=0;j<H;j++) {
				dataIn[slice][i][j] = 0;
				if( data[slice][i][j] != dataOut[slice][i][j] ) dataIn[slice][i][j] = thresholdSpec.newValue;
			}
		}
	...
	printf("%s, '%s' : %d pixels dilated\n", argv[0], inputFilename, totalPixelsDilated);
